...
class Data(object):
    def __init__(self, path, batch_size, task_name):
        self.path = path
        self.batch_size = batch_size
        self.task_name = task_name

        if task_name == 'reconstruction':
            train_file = path + '/mask_train.txt'
        elif task_name == 'downstream_task':
            train_file = path + '/train.txt'
        elif task_name == 'contrastive_delete':
            train_file = path + '/delete_train.txt'
        elif task_name == 'contrastive_replace':
            train_file = path + '/replace_train.txt'
        else:
    ...
    def get_adj_mat(self):
        if self.task_name ==  'reconstruction':
        ...
        elif self.task_name == 'downstream_task':
            adj_mat, norm_adj_mat, mean_adj_mat = self.create_adj_mat()
            adj_mat = adj_mat
            rowsum = np.array(adj_mat.sum(1))
            d_inv = np.power(rowsum, -0.5).flatten()
            ...
            pre_adj_mat = norm_adj.tocsr()
            return adj_mat, norm_adj_mat, mean_adj_mat, pre_adj_mat
        elif self.task_name == 'contrastive_delete':
            adj_mat, norm_adj_mat, mean_adj_mat = self.create_adj_mat()
            adj_mat = adj_mat
            rowsum = np.array(adj_mat.sum(1))
            d_inv = np.power(rowsum, -0.5).flatten()
        ...
        elif self.task_name == 'contrastive_replace':
            adj_mat, norm_adj_mat, mean_adj_mat = self.create_adj_mat()
            adj_mat = adj_mat
            rowsum = np.array(adj_mat.sum(1))
            d_inv = np.power(rowsum, -0.5).flatten()
    ...
    def create_adj_mat(self):
        ...
        def normalized_adj_single(adj):
            ...
            d_inv[np.isinf(d_inv)] = 0. # 无穷大已经归为0了
            d_mat_inv = sp.diags(d_inv)
