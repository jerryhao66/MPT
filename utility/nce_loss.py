...
def nt_xent(z1, z2, batch_size, temperature, zdim):
    ...
    z_ = tf.reshape(tf.transpose(tf.reshape(z, [2, batch_size, zdim]), [1,0,2]), [batch_size*2, -1]) # N*2, D
    ...
    pos_mask = tf.tensor_scatter_add(pos_mask, pos_indices, tf.ones((batch_size*2, 1), dtype=tf.int32))
